...
class SecurityAnalystApp {
    ...
    async checkApiConnection() {
        try {
            ...
            if (response.ok) {
                ...
                // Set connected status if we got a valid response
                this.apiKey = data.demo_api_key || 'demo';
                document.getElementById('api-key').value = this.apiKey;
                this.setConnectionStatus(true, data.mode || 'production');
                
                // Initialize stats with demo values if in demo mode
                if (data.mode === 'demo') {
                    this.stats.queriesToday = 12;
                    this.stats.avgResponseTime = 245;
                    this.stats.threatsDetected = 3;
                    this.updateStats();
                }
            } else {
    ...
    updateStatsAfterQuery(responseTime, data) {
        ...
        // Handle both production and demo response formats
        if (data.technical_details && data.technical_details.threats_found) {
            this.stats.threatsDetected += data.technical_details.threats_found;
        } else if (data.threats_found) {
            // Demo mode might return threats_found at top level
            this.stats.threatsDetected += data.threats_found;
        }
