...
Supports demo mode for unauthenticated access when AWS is unavailable.
"""
...
# Import AWS availability detector and AuthMode
from aws_bedrock_athena_ai.api.aws_detector import AWSAvailabilityDetector, AuthMode
...
# Global AWS availability detector
...
def get_auth_mode() -> AuthMode:
    ...
    Returns:
        AuthMode.DEMO if AWS Bedrock is unavailable or DEMO_MODE=true
        AuthMode.PRODUCTION if AWS Bedrock is available
    """
    if aws_detector.is_bedrock_available():
...
async def optional_auth(request: Request) -> Dict:
    """
    Optional authentication - allows unauthenticated access in all modes.
    
    This is used for public endpoints that work without authentication.
    If authentication is provided, it will be validated and used.
    If not provided, returns a guest user context.
    
    Returns:
        User data dict with mode indicator and permissions
    """
    ...
    # If no auth header, allow as guest
    if not auth_header:
        logger.debug("No authentication provided - allowing as guest")
        return {
            "user_id": "guest_user",
            "permissions": ["query", "read"],
            "mode": auth_mode.value if hasattr(auth_mode, 'value') else str(auth_mode),
            "authenticated": False
        }
    ...
    try:
        ...
        if not auth_header.startswith("Bearer "):
            # Invalid format, but allow as guest
            return {
                "user_id": "guest_user",
                "permissions": ["query", "read"],
                "mode": auth_mode.value if hasattr(auth_mode, 'value') else str(auth_mode),
        ...
        if not key_data:
            # Invalid key, but allow as guest
            return {
                "user_id": "guest_user",
                "permissions": ["query", "read"],
                "mode": auth_mode.value if hasattr(auth_mode, 'value') else str(auth_mode),
        ...
        if not auth_manager.check_rate_limit(key_hash):
            raise HTTPException(
                status_code=429,
