...
class SecurityMiddleware:
    ...
    def _authenticate_with_api_key(self, request: Request, 
        ...
        if not credentials:
            # Allow unauthenticated access as guest user
            logger.debug("No credentials provided - creating guest user context")
            
            principal = IAMPrincipal(
                principal_type="guest",
                principal_id="guest_user",
                arn="arn:aws:iam::demo:guest/guest_user",
                account_id="demo",
                user_name="guest"
            )
