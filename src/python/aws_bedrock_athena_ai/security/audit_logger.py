...
class AuditLogger:
    ...
    def log_security_query(self, user_id: str, question: str, session_id: str, timestamp: datetime):
        """Log a security query request."""
        logger.info(f"Security query logged: user={user_id}, session={session_id}, question_hash={self._hash_query(question)}")
    
    def log_analysis_completion(self, session_id: str, user_id: str, threats_found: int, cost_usd: float, processing_time_ms: float):
        """Log successful analysis completion."""
        logger.info(f"Analysis completed: session={session_id}, user={user_id}, threats={threats_found}, cost=${cost_usd:.4f}, time={processing_time_ms:.1f}ms")
    
    def log_analysis_error(self, session_id: str, user_id: str, error_message: str, processing_time_ms: float):
        """Log analysis error."""
        logger.error(f"Analysis failed: session={session_id}, user={user_id}, error={error_message}, time={processing_time_ms:.1f}ms")